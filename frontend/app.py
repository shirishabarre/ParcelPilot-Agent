...
if not st.session_state.session:
    ...
    with right:
        ...
        selected_user = st.selectbox(
            "Select identity",
            users,
            format_func=lambda user: labels.get(user, user)
        )
    ...
    with col2:
        ...
        st.write(
            "Retrieve the relevant policy or support documentation."
        )
    ...
    with col3:
        ...
        st.write(
            "Investigate a support ticket and identify the likely next step."
        )

        if st.button(
            "Investigate Ticket",
            key="investigate_ticket",
            use_container_width=True
        ):
            open_chat(
                "Please investigate my latest support ticket "
